...
class ContextReport:
    ...
    # mood
    mood: str = "neutral"
    mood_dist: Optional[Dict[str, float]] = None
    ...
    def render(self) -> str:
        ...
        if self.surface == "agent" or self.tools_tokens:
            lines.append(row("system tools", self.tools_tokens,
                             f"({self.tools_available} available)"))
            lines.append(row("skills", self.skills_tokens,
                             f"({self.skills_available} available)"))
        lines.append(row("memory", self.memory_tokens,
                         f"({self.memory_files} experiences)"))
        ...
        lines.append(f"  {'window':<17}{self.used:>6}/{self.context_len} "
    ...
    def render_compact(self) -> str:
        ...
        r = (f"in {self.tokens_in} · out {self.tokens_out} · think {self.tokens_reasoning}"
