...
data sources, lives in its own package under src/plugins/<domain>/stageNN_<slug>/, and is
...
@dataclass(frozen=True)
class DomainSpec:
    """How the framework BUILDS, TRAINS and EVALUATES one domain's model — the seam
    that makes the engine task/modality-agnostic. The default (a text LLM) lives in
    `src/core/training/domain.py`; a domain package overrides it by exposing a module-
    level `DOMAIN = DomainSpec(...)` (or `build_domain(cfg) -> DomainSpec`), so the same
