...
from src.core.memory.experience_log import EXPERIENCE_LOG, load_experiences
from src.core.memory.ltss import LTSS

# Recalled memory rides into EVERY surface (chat, agent, future API) as a leading
# `<mem>…</mem>` block — the SAME framing the Memory-management stage (stage 6) is
# trained on, so the frozen core consumes it in-distribution. Owned by core (the read
# side of memory); the consumers in uses/ import `memory_block` from here.
...
def memory_block(body: str) -> str:
    ...
    Empty body → "" (no injection, zero regression)."""
    body = (body or "").strip()
    return f"{MEM_OPEN}\n{body}\n{MEM_CLOSE}\n" if body else ""
...
class MemoryRecall:
    ...
    def __init__(
        ...
        self._exp_cache: tuple | None = None  # (mtime, [(text, emb)])

    def _open_ltss(self) -> LTSS | None:
        try:
    ...
    def _experience_embeddings(self) -> list[tuple]:
        ...
        for rec in load_experiences(path):
            text = (rec.get("text") or rec.get("response") or rec.get("prompt") or "").strip()
            emb = self.embed(text)
            if emb is not None:
                out.append((text, emb))
