...
class SectorSnapshotManager:
    ...
    def snapshot_before_update(
        self, sector_id: int, sector_params: dict, cycle_t: float | None = None
    ) -> Path:
    ...
    def rollback(self, sector_id: int, sector_adapter, cycle_t: float | None = None) -> bool:
    ...
    def detect_catastrophe(
        ...
        grad_spike = len(hist) >= 10 and grad_norm > _mean(hist) + GRAD_SPIKE_SIGMA * _std(hist)
        ...
        if triggered:
            ...
            logging.warning(f"CAT sector {sector_id}: " + " | ".join(r for r in reasons if r))
        else:
