...
(`src.core.backend.current()`), so the same code runs on MLX or PyTorch. Select the
backend (via `src.core.backend.select`) BEFORE importing this module.
"""

from __future__ import annotations

import src.core.backend as backend
from src.core.model.blocks import (
    RMSNorm,
    TransformerBlock,
    apply_rope,  # noqa: F401  (re-exported for compatibility)
)
from src.core.model.config import ModelConfig  # re-exported below for compatibility
...
class RDMCAFoundational(nn.Module):
    ...
    def __init__(self, cfg: ModelConfig):
        ...
        self.ple = None
        if cfg.ple_dim > 0:
            from src.core.model.ple import PerLayerEmbeddings

            self.ple = PerLayerEmbeddings(cfg)
        self.mtp = None
        if cfg.n_mtp_heads > 0:
            from src.core.model.mtp import MTPModule
    ...
    def add_sector(self, sector_id: int, rank: int = 4):
        ...
        from src.core.model.config import LoRAConfig
        from src.core.model.lora import SectorAdapter
    ...
    def _route(self, x):
        ...
        if self.gate is None or self._routing.mode != "moe":
            return None
        from src.core.model.moe import expert_weights, load_balance_loss
