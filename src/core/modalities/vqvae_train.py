...
def train_image_tokenizer(
    ...
    from src.core.modalities.image import _resize
...
def train_audio_tokenizer(
    ...
    if audio_dir and Path(audio_dir).is_dir():
        from src.core.modalities.perception import load_audio
    ...
    if backend_name:
        backend.select(backend_name)
    B = backend.current()
    from src.core.modalities.audio import AudioVQVAE, logmel
