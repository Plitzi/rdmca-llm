...
A **model** is one training scenario living under `src/models/<name>/` (e.g.
...
`src/models/<name>/stage11_*/` and it joins that model's curriculum with no other
...
from src.models.base import StagePlugin
...
def set_active_model(name: str | None) -> None:
    """Select the active model (the `src/models/<name>/` package whose stages the
...
def _discover(model: str) -> dict[int, StagePlugin]:
    ...
    pkg = importlib.import_module(f"src.models.{model}")

    found: dict[int, StagePlugin] = {}
    for mod in pkgutil.iter_modules(pkg.__path__):
        if not _PACKAGE_RE.match(mod.name):
            continue
        plugin = importlib.import_module(f"src.models.{model}.{mod.name}").PLUGIN
...
def stage_data_dir(number: int, cfg: dict | None = None) -> str:
    ...
    its own package (`src/models/<model>/stageNN_<slug>/data/level{L}/`), so a stage
    ...
    base = f"src/models/{_ACTIVE_MODEL}/{package}/data"
