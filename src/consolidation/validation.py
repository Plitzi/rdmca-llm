    ...
    def assess(self, experience, coherence: float) -> ValidationResult:
        ...
        if fb in ("corrected", "accepted"):
            return ValidationResult(
                "approve",
                self.human_feedback_confidence,
                self.name,
                f"user-{fb}: authoritative ground truth",
            )
        conf = max(0.0, min(1.0, float(coherence)))
        return ValidationResult(
            "approve", conf, self.name, "consistency with prior knowledge (LTSS coherence)"
        )
...
class ExperienceValidator:
    ...
    def __init__(
        self,
        self_source: SelfKnowledgeSource | None = None,
        external: list[KnowledgeSource] | None = None,
        human_source: HumanReviewSource | None = None,
        high: float = 0.66,
        low: float = 0.33,
    ):
    ...
    def decide(self, experience, coherence: float) -> Decision:
        ...
        if s.confidence < self.low:
            if self.human_source is not None:
                self.human_source.enqueue(
                    experience, s.confidence, "low self-confidence, no external source resolved it"
                )
...
def default_validator(
    ambiguity_handler=None, peer_client=None, research_tool=None
) -> ExperienceValidator:
