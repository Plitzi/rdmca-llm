...
def _build_target(prompt: str, response: str, feedback: str,
...
def log_experience(prompt: str, response: str = "", feedback: str = "neutral",
                   correction: Optional[str] = None, lang: str = "en",
                   modality: str = "text", path: str = EXPERIENCE_LOG) -> bool:
    ...
    with open(p, "a", encoding="utf-8") as f:
        f.write(json.dumps({
            "text": text, "prompt": prompt, "response": response,
            "feedback": feedback, "correction": correction,
            "lang": lang, "modality": modality, "timestamp": time.time(),
        }, ensure_ascii=False) + "\n")
    return True
