...
TOOL_SPECIALS: list[str] = ["<tool_call>", "</tool_call>", "<tool_response>", "</tool_response>"]
...
def tokenizer_symbols(langs: list[str]) -> list[str]:
    ...
    return [f"<lang:{l}>" for l in langs] + list(MODALITY_SPECIALS) + list(CONTROL_SPECIALS)


def build_modality_layout(
    text_vocab_size: int,
