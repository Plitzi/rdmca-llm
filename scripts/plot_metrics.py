...
def _entry_ppl(gate) -> float | None:
...
def plot_stage(csv_path: Path, out: Path | None, label: str, plt) -> Path | None:
    ...
    if has_replay:
        ...
        if rx:
            ax.plot(rx, ry, color="#d62728", lw=1.0, alpha=0.8, label="conversation rehearsal")
        if sx or rx:
    ...
    if xs:
        ...
        ax.set_yscale("log")
    ax.set_title("Live per-token perplexity")
    ax.set_xlabel("tokens (M)")
    ax.set_ylabel("ppl (log)")
...
def plot_overview(level: int, out: Path | None, plt, model: str = "cognition") -> Path | None:
    ...
    for s in stages:
        ...
        if sc.exists():
            try:
                ...
                pass
        if best is None:
            _bxs, bys = _series(gate, "step", "best")
            best = bys[-1] if bys else None
        per.append((s, entry, best, gate))

    fig, (ax0, ax1) = plt.subplots(2, 1, figsize=(13, 9))
    fig.suptitle(
        f"RDMCA — full curriculum panorama (level {level})", fontsize=14, fontweight="bold"
    )
    ...
    w = 0.38
    ax0.bar(
        [x - w / 2 for x in xs], entries, width=w, color="#7f7f7f", label="entry PP (inherited)"
    )
    ax0.bar([x + w / 2 for x in xs], bests, width=w, color="#2ca02c", label="best PP (stage end)")
    for x, (_s, e, b, _) in zip(xs, per, strict=False):
        if e and b and e > 0:
            ax0.annotate(
                f"{(b - e) / e * 100:+.0f}%",
                (x, max(e, b)),
    ...
    ax0.set_xticks(xs)
    ax0.set_xticklabels([f"S{s}" for s, *_ in per])
    ax0.set_ylabel("val perplexity")
    ax0.set_title("Per-stage entry vs final best PP (Δ% = offset-corrected change)")
    ...
    for s, _, _, gate in per:
        ...
        ticks.append(offset + (gx[-1] / 2 if gx else 0))
        tlabels.append(f"S{s}")
        offset += (gx[-1] if gx else 0) + max(1, (gx[-1] if gx else 0) * 0.04)
    ax1.set_xticks(ticks)
    ax1.set_xticklabels(tlabels)
    ax1.set_ylabel("gate val ppl")
    ax1.set_title("Gate validation perplexity across the curriculum")
    ax1.grid(alpha=0.3)
