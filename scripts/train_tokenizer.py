...
def _vqvae_train_loop(B, console, model, sample_batch, steps, batch, lr, out, label):
    ...
    for step in range(1, steps + 1):
        ...
        if step % 200 == 0 or step == 1:
            console.print(f"    step {step:5d} | loss {B.engine.item(loss):.4f} | "
...
def train_image_tokenizer(console, images_dir, dataset, n, img_size,
                          steps, batch, lr, out, backend_name=None) -> bool:
        ...
        try:
            ...
            for ex in ds:
                if key is None:
                    ...
                    if key is None:
                        console.print(f"  [yellow]image: dataset '{dataset}' has no "
            ...
            return False
    if not arrs:
        console.print("  [dim]image: no data (pass --images-dir or --image-dataset) — skipping[/dim]")
        return False
...
def train_audio_tokenizer(console, audio_dir, synthetic, n,
                          steps, batch, lr, out, backend_name=None) -> bool:
    ...
    if audio_dir and Path(audio_dir).is_dir():
        ...
        for _ in range(n):
            ...
            wav = (0.5 * np.sin(2 * np.pi * f1 * t) + 0.3 * np.sin(2 * np.pi * f2 * t)
                   + 0.05 * np.random.randn(t.size))
            clips.append(wav.astype(np.float32))
    if not clips:
        console.print("  [dim]audio: no data (pass --audio-dir or --audio-synthetic) — skipping[/dim]")
        return False
...
def main():
    ...
    parser.add_argument("--audio-synthetic", action="store_true",
    ...
    parser.add_argument("--backend", default=None, choices=["mlx", "torch"],
    ...
    if not args.text_only:
        ...
        train_image_tokenizer(console, args.images_dir, args.image_dataset, args.mm_n,
                              args.mm_img_size, args.mm_steps, args.mm_batch, args.mm_lr,
                              IMAGE_OUT, backend_name=args.backend)
        train_audio_tokenizer(console, args.audio_dir, args.audio_synthetic, args.mm_n,
                              args.mm_steps, args.mm_batch, args.mm_lr,
                              AUDIO_OUT, backend_name=args.backend)
