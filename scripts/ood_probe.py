...
from src.modalities.text import BOS_ID  # noqa: E402
...
def answer(model, mcfg, tokenizer, prompt, lang="en", max_new=64, temperature=0.0):
    ...
    ids, _ = generate(
        model, gen_history, max_new_tokens=max_new,
        temperature=temperature, top_p=(1.0 if temperature == 0 else 0.9),
        vocab_size=mcfg.vocab_size, context_len=mcfg.context_len, stream=False,
        decode_fn=tokenizer.decode, top_k=(1 if temperature == 0 else 0),
        rep_penalty=1.0, stop_strings=agent.ANSWER_STOP_STRINGS,
    )
...
def temp_robustness(model, mcfg, tokenizer, temps=(0.0, 0.8, 1.0), n=12, seed=7):
    ...
    print(f"  answer stable across temps: {stable}/{n}  "
...
def main():
    ...
    arithmetic = [
        "What is 12 + 7?", "What is 48 + 27?", "What is 90 - 13?",
        "What is 6 + 9?", "What is 345 + 678?", "What is 53 - 28?",
    ...
    conversation = [   # retention check — must still talk, not collapse to the skill
        "Hi, how are you?", "What is your name?", "Tell me about dogs.",
    ]

    sets = {2: ("ANALOGIES (OOD)", analogies),
