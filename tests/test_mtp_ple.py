...
def _cfg(**kw):
    base = dict(d_model=64, n_layers=3, n_heads=4, n_kv_heads=2, ffn_dim=128,
                context_len=64, vocab_size=256, mrl_dims=[32, 64], dropout=0.0)
    base.update(kw)
...
def test_mtp_zero_init_is_noop_at_start():
    ...
    B, S, V = logits.shape
    ce = float(mx.mean(nn.losses.cross_entropy(
        logits.reshape(B * S, V),
        mx.array(np.random.randint(0, V, (B * S,))))))
...
def test_ple_trains_and_diverges_from_noop():
    ...
    np.random.seed(4); mx.random.seed(4)
    cfg = _cfg(ple_dim=8)
    m = RDMCAFoundational(cfg)
    toks = mx.array(np.random.randint(0, 256, (2, 16)))
    opt = optim.Adam(learning_rate=3e-3)
...
def test_ple_cached_matches_full():
    ...
    for _ in range(6):
        _, grads = lg(m); opt.update(m, grads); mx.eval(m.parameters(), opt.state)
    ...
    assert np.allclose(full, cached, atol=1e-3), \
        f"max abs diff {np.abs(full - cached).max():.2e}"
