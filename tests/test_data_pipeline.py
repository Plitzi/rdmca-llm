...
def test_validation_perplexity_routes_mask_and_bare(tmp_path):
    ...
    mask = np.zeros((2, 33), dtype=np.int32)
    mask[:, 20:] = 1  # only the tail trains
    B = backend.current()
    ...
    exp_masked = float(np.exp(B.engine.item(m.eval_ce(B.ops.array(toks), mask=B.ops.array(mask)))))
...
def test_evaluate_gate_respects_threshold_and_config_override():
    ...
    ppl0, _ = T.evaluate_gate(
        m, 1, val, {"gate": {"max_perplexity": {1: 1e9}}}, log=lambda *a, **k: None
    )
    # A threshold just above the measured ppl passes; just below fails.
    _, passed_hi = T.evaluate_gate(
        m, 1, val, {"gate": {"max_perplexity": {1: ppl0 + 1}}}, log=lambda *a, **k: None
    )
    _, passed_lo = T.evaluate_gate(
        m, 1, val, {"gate": {"max_perplexity": {1: ppl0 - 1}}}, log=lambda *a, **k: None
    )
