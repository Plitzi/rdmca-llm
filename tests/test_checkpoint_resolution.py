...
from uses.chat.run_chat import resolve_stage_checkpoint, describe_checkpoint_meta
...
def test_prefers_best_over_final_and_latest(tmp_path):
    ...
    (tmp_path / "best.json").write_text(json.dumps(
        {"score": 14.2, "step": 7920, "tokens": 64_000_000}))
    (tmp_path / "latest.json").write_text(json.dumps(
        {"checkpoint": str(tmp_path / "step_00008000.npz"), "step": 8000}))
...
def test_falls_back_to_final_when_no_best(tmp_path):
    _touch(tmp_path / "final.npz")
    (tmp_path / "stage_complete.json").write_text(json.dumps(
        {"gate_score": 11.0, "step": 9000, "met_bar": True}))
    path, label, meta = resolve_stage_checkpoint(tmp_path)
...
def test_falls_back_to_latest_in_progress(tmp_path):
    ...
    (tmp_path / "latest.json").write_text(json.dumps(
        {"checkpoint": str(tmp_path / "step_00002000.npz"), "step": 2000,
...
def test_latest_pointing_at_missing_file_is_ignored(tmp_path):
    (tmp_path / "latest.json").write_text(json.dumps(
        {"checkpoint": str(tmp_path / "gone.npz"), "step": 1}))
    assert resolve_stage_checkpoint(tmp_path)[0] is None


def test_describe_checkpoint_meta_formats_quality():
    desc = describe_checkpoint_meta({"score": 14.2, "step": 7920,
                                     "tokens": 64_000_000, "met_bar": True})
