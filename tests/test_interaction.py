...
from src.agent import run_agent, Tool
...
def test_run_agent_injects_steering_correction():
    ...
    noop = Tool(name="noop", description="no-op", input_schema={},
                run=lambda _inp: {"ok": True})
    res = run_agent(gen, [noop], "convert 5 miles", max_steps=4,
