...
def test_validator_external_stubs_are_inert_until_configured():
    ...
    from src.consolidation.validation import PeerModelSource, WebResearchSource, default_validator
...
def test_agent_think_delimiters_match_registry():
    from src.agent import THINK_CLOSE, THINK_OPEN
...
def test_loader_interleaves_sources_no_block():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        _write_corpus(d)
        ds = TextDataset(
            str(d), _FakeTok(), seq_len=16, batch_size=2, shuffle=True, shuffle_buffer=50
        )
        seen = []
...
def test_loader_with_mask_yields_token_mask_pairs():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        ds = TextDataset(
            str(d), _FakeTok(), seq_len=16, batch_size=2, shuffle=False, with_mask=True
        )
...
def test_emotion_maps_to_mood_palette():
    from src.modalities.moods import MOODS, emotion_to_mood
...
def test_mood_tracker_builds_and_decays_over_conversation(monkeypatch):
    ...
    from src.model.mood import MOOD_INDEX, MOODS, MoodTracker
...
def test_mood_head_learns_to_separate_moods():
    ...
    from src.model.mood import MOOD_INDEX, MoodHead, mood_loss
