...
def _corpus(tmp_path: Path) -> Path:
    ...
    lines = [json.dumps({"text": "".join(r.choice(alpha) for _ in range(r.randint(5, 60)))})
             for _ in range(3000)]
...
def _loader(data_dir: Path, tok_path: Path, seed=1234) -> DataLoader:
    ds = TextDataset(str(data_dir), _VarTok(tok_path), seq_len=8, batch_size=2,
...
def test_fast_skip_matches_plain_stream(tmp_path):
    ...
    offsets = [o for o in range(K, K + 8)
