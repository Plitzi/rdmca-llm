...
class _StubLTSS:
    ...
    def __init__(self, coh): self._coh = coh
    def max_cosine_similarity(self, emb): return self._coh
...
def test_empty_cycle_is_all_zero():
    ...
    assert gm == {"saturation": 0.0, "exc_rate": 0.0,
...
def test_metrics_can_drive_pgq_off_stable():
    ...
    res = pgq.evaluate("c1", saturation=0.9, exc_rate=0.9, pred_error=0.9,
                       cluster_novel=0.9, busiest_sector_id=1, sectors={}, model=None)
    assert res.decision != "stable" and res.gns > P.PGQ_SAT_REF * 0  # gns>0
    # all-zero (old behavior) → stable
    res0 = pgq.evaluate("c0", saturation=0.0, exc_rate=0.0, pred_error=0.0,
                        cluster_novel=0.0, busiest_sector_id=1, sectors={}, model=None)
