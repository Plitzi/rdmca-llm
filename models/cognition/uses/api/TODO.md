...
- **Tool loop** — `src/agent.py`: `Tool`, `run_agent()` (the agentic loop already
  used by [`models/cognition/uses/agent/run_agent.py`](../agent/run_agent.py)).
- **Tools / skills** — discover from `models/cognition/uses/agent/tools/` and `models/cognition/uses/agent/skills/`.
