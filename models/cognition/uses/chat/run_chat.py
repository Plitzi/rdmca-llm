...
try:
    ...
    _repo = os.path.dirname(  # models/cognition/uses/chat/run_chat.py → repo (5 up)
        os.path.dirname(
...
from models.cognition.mood import MOODS
...
# lives in models/cognition/uses/common/generate.py so the chat and agent runtimes share it.
# Re-exported here as the shared import surface (run_agent + tests import these via run_chat).
from models.cognition.uses.common.generate import (  # noqa: F401  (shared import surface: tests + run_agent)
    GEN_DEADLINE_S,
    IncrementalDecoder,
    _looping,
    generate,
...
Examples:
  ...
  python models/cognition/uses/chat/run_chat.py --level 1 --stage 1 --no-mood   # always neutral, just answer
    ...
    if not args.dummy and args.stage is None and args.checkpoint is None:
        ...
        print("Example: python models/cognition/uses/chat/run_chat.py --dummy")
        sys.exit(1)
